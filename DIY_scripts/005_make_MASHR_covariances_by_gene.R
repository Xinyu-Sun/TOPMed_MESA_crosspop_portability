...
# example of --snpdosage:
# "../my_input/processed_input/whole_blood_"
parser$add_argument('-d', '--snpdosage', help='[Prefix] file path of the unfilterd SNP dosage file')
parser$add_argument('-g', '--geneannotation', help='file path of the gene annotation file that contains `ALLchr`')
parser$add_argument('-t', '--tag', help='file tag for this run of samples; Need to have {ancestry}_{tag} format')
#parser$add_argument('-c', '--chromosome', help='chromosome to be analyzed')
parser$add_argument("--gene", help="gene to be analyzed. Use ENSG ID", type='character')
...
### 
print('INFO: '%&% args$tag %&%' '%&% args$gene)
# get ancestry from tag
# tag format is `ancestry_tag`
ancestry <- strsplit(args$tag, '_')[[1]][1]
print('INFO: Loading files')
# Get *THE* gene to compute covariance for
gene_annot <- fread(args$geneannotation, header=T, stringsAsFactors=F) %>% filter(gene_id==args$gene)
genes_in_model <- fread(args$model, fill=T) %>% pull(gene) %>% unique()
list_genes <- intersect(gene_annot$gene_id, genes_in_model)

chr_num <- gene_annot$chr

# fill in -snpdosage path with ancestry and chromosome
# example of snpdosage path:
# "../my_input/processed_input/whole_blood_${pop}_chr${chr}/snp_dosage.txt"
snpdosage_path <- args$snpdosage %&% ancestry %&% '_chr' %&% gene_annot$chr %&% '/snp_dosage.txt'

# Read dosage file (unfiltered!)
genotype_file <- fread(snpdosage_path, header=T, stringsAsFactors=F)
...
# Make empty cov file
covariance_file <- args$outputdir %&% '/' %&% args$tag %&%'_'%&% args$gene %&% '_covariances.txt'
